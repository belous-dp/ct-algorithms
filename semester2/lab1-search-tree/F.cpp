...
struct node {
    ...
    node* left;
    node* right;
    node(int val) : val(val), priority(rnd()), size(1), left(nullptr), right(nullptr) { }
};
using pnode = node*;
...
void split(pnode root, int key, pnode& left, pnode& right, int size_sum = 0) {
    if (!root) {
        left = right = nullptr;
        return;
    }
    int root_key = get_size(root->left) + size_sum + 1;
    if (root_key <= key) {
        split(root->right, key, root->right, right, root_key);
        left = root;
    } else {
        split(root->left, key, left, root->left, size_sum);
        right = root;
    }
    upd_size(root);
}
